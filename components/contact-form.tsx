...
export default function ContactForm() {
  ...
  // Cargar y renderizar Google reCAPTCHA v2 (checkbox)
  useEffect(() => {
    const siteKey = (process.env.NEXT_PUBLIC_RECAPTCHA_KEY as string) || (process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY as string) || '6LfqGiYsAAAAAPuFBpHESQp-ijfAMezRQ9Jt47VE'
    if (!siteKey) return

    const w = window as any
    const render = () => {
      if (!recaptchaRef.current) return
      if (w.grecaptcha && !widgetIdRef.current) {
        widgetIdRef.current = w.grecaptcha.render(recaptchaRef.current, {
          sitekey: siteKey,
    ...
    const id = 'google-recaptcha-v2'
    if (!document.getElementById(id)) {
      const s = document.createElement('script')
      s.id = id
      s.src = 'https://www.google.com/recaptcha/api.js?onload=__recaptchaLoaded&render=explicit'
      s.async = true
      s.defer = true
      ;(w as any).__recaptchaLoaded = () => {
        render()
      }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!recaptchaToken) {
      setStatus('error')
      setErrorMsg('Por favor completa el reCAPTCHA antes de enviar.')
      return
    }
    ...
    try {
      ...
      const res = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({ to, subject, text, recaptchaToken, replyTo: email }),
      })
