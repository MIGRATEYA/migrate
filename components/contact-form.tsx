...
export default function ContactForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(`Consulta - ${fullName || 'Sin nombre'}`)
    const body = encodeURIComponent(`Nombre: ${fullName}\nEmail: ${email}\nTeléfono: ${phone}\nPaís: ${country}\n`)
    window.location.href = `mailto:info@migrateya.com?subject=${subject}&body=${body}`
  }
          ...
          <form onSubmit={onSubmit} className="max-w-3xl mx-auto space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input id="firstName" className="form-input w-full placeholder-gray-400" type="text" placeholder="Nombre..." required value={firstName} onChange={(e) => setFirstName(e.target.value)} />
              </div>
              <div>
                <label className="sr-only" htmlFor="lastName">Apellido</label>
                <input id="lastName" className="form-input w-full placeholder-gray-400" type="text" placeholder="Apellido..." required value={lastName} onChange={(e) => setLastName(e.target.value)} />
              </div>
            </div>
            <div>
              <label className="sr-only" htmlFor="email">Correo</label>
              <input id="email" className="form-input w-full placeholder-gray-400" type="email" placeholder="Correo electrónico.." required value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div>
              <label className="sr-only" htmlFor="phone">Teléfono</label>
              <input id="phone" className="form-input w-full placeholder-gray-400" type="tel" placeholder="Teléfono" value={phone} onChange={(e) => setPhone(e.target.value)} />
            </div>
            <div>
            ...
            <div className="flex items-center justify-end">
              <button type="submit" className="inline-flex items-center px-6 py-3 rounded-lg text-gray-900 bg-blue-400 hover:bg-yellow-500 font-medium shadow-md transition-colors">
                Enviar
